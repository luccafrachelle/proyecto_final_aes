...
original_tree <- rpart(
  formula = exito ~ ., 
  data = train %>% select(-c(dataset, exito_num)), 
...
- El árbol de decisión inicia evaluando la variable `n_eventos`, que es el factor principal para predecir el resultado. Si `n_eventos` es mayor o igual a 6, se toma la rama izquierda; de lo contrario, la derecha.

- En la rama izquierda (`n_eventos >= 6`), el siguiente factor es `nzonas`. Cuando `nzonas` es mayor o igual a 3, aumenta la probabilidad de éxito (`1`). Si es menor, la probabilidad de éxito disminuye.

- En la rama derecha (`n_eventos < 6`), el modelo considera `vert_tot`. Si `vert_tot >= 50`, se evalúa `dist.promP`, donde valores menores a 2.8 aumentan la probabilidad de éxito.

- Si `vert_tot < 50`, se analiza nuevamente `n_eventos`. Si es mayor o igual a 4, la probabilidad de éxito depende de `vert_tot < -42`; de lo contrario, la probabilidad de fracaso (`0`) aumenta.

- Las variables clave en el modelo son `n_eventos`, `vert_tot`, `nzonas`, y `dist.promP`, con interacciones específicas que combinan estos factores para predecir el resultado.

Luego de esto se tunea el modelo para obtener mejores hiperparámetros y se ajusta de nuevo. Como resultado, se obtiene un árbol de decisión más profundo que permite predecir mejor el resultado de las jugadas.
...
De igual manera, se eligen los hiperparámetros que mejoren la curva de ROC AUC.
La elección de los hiperparámetros es la siguiente:
...
```{r}
temp = temp  %>% group_by(team.name , mundial) %>% summarise(cant = n() , error=mean(exito!=.pred_class)) %>% arrange(desc(error)) %>% 
  left_join(ranking,by=c("team.name","mundial"))  %>% 
  mutate(error = 1 - round(error,4)) 
```

```{r}
temp  %>% ggplot(aes(x = team.name, y = error)) + geom_bar(stat = "identity") + labs(y = "% error por equipo", x = "Equipo")  + theme_bw() + 
  coord_flip() + facet_wrap(~mundial)
```

```{r}
temp  %>% ggplot(aes(x = Ranking, y = error)) + geom_point() + labs(y = "Erorr", x = "Ranking")  + theme_bw() + 
   facet_wrap(~mundial) + labs(title = "Error por equipo por mundial")
```

```{r}
temp  %>% ggplot(aes(x = cant, y = error)) + geom_point() + 
  labs(y = "Erorr", x = "Cantidad de jugadas" , title = "Error por equipo por mundial") + theme_bw() + 
  facet_wrap(~mundial)
```
