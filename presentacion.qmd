...
variables_info <- data.frame(
  ...
  Descripcion = c("ID de la posesion","Duracion en segs de la jugada", "Periodo del partido", "ID del equipo en posesion",
                  "Nombre del equipo en posesion", "Nombre del equipo","Cantidad de acciones de la jugada", "Coordenada X de inicio de la jugada",
                  "Coordenada Y de inicio de la jugada", "Coordenada X de fin de la jugada", "Coordenada Y de fin de la jugada",
                  "Numero de eventos distintos", "Numero de pases", "Numero de pases completados",
                  "Numero de traslados", "Promedio de avance por pase realizado", "Promedio de avance por traslado realizado",
                  "Numero de jugadores que participan en la jugada", "Cantidad de centros", "Cantidad de cambios de frente",
                  "Numero de pases dentro del area rival", "Numero de ingresos al area rival",
                  ...
                  "Velocidad media de los pases", "Velocidad media de los traslados","Mundial al que pertenece el partido","Verticalidad total de la posesion","Horizontalidad total de la posesion","Distancia promedio de los pases","Distancia promedio de los traslados","Distancia mediana de los pases","Distancia mediana de los traslados","Cantidad de acciones que se realizan bajo presion del rival","Cantidad de acciones del equipo que tiene la posesion","Exito (o no) de la posesion"),
  
  Tipo = c("Numerica", "Categorica", "Numerica", "Categorica", "Categorica", "Numerica",
           "Numerica", "Numerica", "Numerica", "Numerica", "Numerica", "Numerica", 
           "Numerica", "Numerica", "Numerica", "Numerica", "Numerica", "Numerica",
           "Numerica", "Numerica", "Binaria", "Categorica", "Categorica", "Categorica", 
           "Numerica", "Numerica", "Numerica","Numerica","Numerica","Binaria","Numerica","Numerica","Numerica","Numerica","Numerica","Numerica","Numerica","Numerica" ,"Binaria"), 
  Rango=c("2:283","0:137.71","1:4","","","","2:146","0.4:120.5","0:79.9","0.1:120","0:79.9","1:11","0:51","0:51","0:38","-30.5:104.9","-17.4:56.9","1:17","0:3","0:4","0:3","0:2","0: no inicia golero, 1: inicia golero","'Empatando', 'Ganando','Perdiendo'","1:30","1:30","0:0.92805","-0.72:1067.5","0:2275","M='Masculino',F='Femenino'","-119.6:116.4","-79.90:79.40","0:108.08","0:62.614","0:108.06","0:62.614","0:23","1:142","0: no exito, 1: exito")
)
```
...
```{r}
variables_info_chunk1 <- variables_info[1:15, ]
variables_info_chunk1 %>%
  gt() %>%
  tab_header(
    title = md("**Caracteristicas de las Variables para el Modelo - Parte 1**")
  ) %>%
  cols_label(
...
```{r}
variables_info_chunk2 <- variables_info[16:30, ]
variables_info_chunk2 %>%
  gt() %>%
  tab_header(
    title = md("**Caracteristicas de las Variables para el Modelo - Parte 2**")
  ) %>%
  cols_label(
    Variable = md("**Variable**"),
    Descripcion = md("**Descripcion**"),
...
```{r}
variables_info_chunk3 <- variables_info[31:39, ]
variables_info_chunk3 %>%
  gt() %>%
  tab_header(
    title = md("**Caracteristicas de las Variables para el Modelo - Parte 3**")
  ) %>%
  cols_label(
    Variable = md("**Variable**"),
    Descripcion = md("**Descripcion**"),
...
```{r}
df %>% mutate(x_fin = 120 - x_fin, x_inicio = 120 - x_inicio) %>%
...
## Interpretabilidad del Random Forest: eventos
-   Menor variedad de tipo de eventos en la jugada: mayor probabilidad de éxito
-   Distintos eventos en una misma secuencia pueden indicar que la posesión no está asegurada del todo y por lo tanto menor probabilidad de éxito     
...
## Interpretabilidad del Random Forest: horizontalidad y verticalidad

-   A menor verticalidad de la jugada, mayor probabilidad de éxito en promedio (en valor absoluto)

-   Mientras que a mayor horizontalidad, mayor probabilidad de éxito (en valor absoluto)

-   Pases y conducciones de pelota hacia adelante implican un mayor riesgo de perder la posesión
...
```{r}
set.seed(1805)
pdp_vert <- model_profile(explainer_rf, N = nrow(df), variables = "vert_tot")
plot(pdp_vert) + geom_rug()
```


## Interpretabilidad del Random Forest: coordendas inicio
-   Zona del área rival (x>100) tienen una alta probabilidad de éxito
-   Mayor probabilidad de éxito se da en posesiones que comienzan alrededor de la mitad de la cancha: menor proporción de acciones bajo presión
-   Para la Y tenemos que, en promedio, la probabilidad de éxito no varía demasiado
...
## Interpretabilidad del RandomForest: distancia pases
-   Pases cortos implican un menor riesgo (entre 15 y 30)
-   Mayor distancia de los pases, mayor riesgo de perder la pelota
...
## % Error por equipo
-   No se logra predecir correctamente el éxito de las jugadas de Irán y Costa Rica (masculino) los cuales no se destacan por tener un juega de mucha posesión (menor cantidad de tiempo la pelota que el rival en promedio) 
-   Jugaron en la fase de grupos contra equipos poderosos y con un estilo de control del juego 
- Brasil, Dinamarca y Países Bajos (femenino) los equipos con mayor % de error
...
## Verticalidad total y horizontalidad total
- Se toman en cuenta los promedios de la horizontalidad y verticalidad total de cada una de las secuencias de cada uno de los 64 equipos
- El modelo no logra captar correctamente el éxito de Brasil (femenino): poca verticalidad y bastante horizontalidad
...
## Modelos separados vs Modelo General
-   No se observan grandes diferencias en la importancia de las variables tanto en los modelos por separado como el unificado
-   % de error parece ser menor en modelo general  